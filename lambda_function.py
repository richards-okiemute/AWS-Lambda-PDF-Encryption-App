...
def encrypt_pdf(file_path, encrypted_file_path):
    ...
    # Retrieve the encryption password from the environment variable
    password = os.environ.get("PDF_ENCRYPTION_PASSWORD")
    if password:
        writer.encrypt(password)  # Correct method to set a password for encryption
    else:
        raise Exception("PDF password not found")
